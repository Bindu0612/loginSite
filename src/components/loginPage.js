...
class LoginPage extends Component {
    ...
    validateValues = () => {
        ...
        for (var i = 0; i <= this.props.users.length-1; i++) 
        {
            
            if (loginEmail == this.props.users[i].EmailId
    ...
    render() {
        return (
            <Form className="form-style">
                ...
                <Button className="btn-lg btn-dark btn-block" onClick={() => this.validateValues()}>Login</Button>
                <div className="text-center">
                    <a style={{textDecoration:"underline",color:"black", cursor:"pointer"}} onClick={() => this.navigateToSignupPage("signUpPage")}>Sign-up</a>
                    <span className="p-2"> | </span>
                        <a  style={{textDecoration:"underline",color:"black", cursor:"pointer"}} > Forgot Password</a>
                </div>
            </Form>
        )
    }
